...
char* BinaryToText(char * bitsToConvert) {
    /** converts each binary bit into an ASCII char
     * and writer the char to the screen as well as to an output file
     * */
    char * string = malloc(sizeof(char) * (int)(strlen(bitsToConvert)/8));
    char s[(int)(strlen(bitsToConvert)/8)];
    char * inputBuffer;
    char temp;
    int start = 0, end = 7;
